...
The story headline is "A not-so-spooky interactive fiction."
The story genre is "Adventure/Supernatural". 
The story description is "Being timid and faint of heart, you wish to prove yourself as someone who is brave by spending the weekend at a haunted campsite. What could go wrong, amirite?".
Release along with cover art [orignially from vnutz.com, edited by me."] ("Cowardice")
...
[player begins game timid
player is accomplished after giving luigi canteen
player is brave after taking wooden key
player is persistent after knocking on ouija board 3 times (commencing)
player is tested after examining name cardd
player is courageous after breaking down attic door]

[I wanted to include moods because they are an easy way to trigger events. Also, it was fun kind of building the player behind the scenes with how they change throughout the game in their emotions. I got the code for moods from Kaitlyn Poentis' Stranded.]
...
Forest Clearing is a room. "Here there is a lack of trees, giving a view of the stars. In the center of the clearing, there is an oddly symmetrical pond. You can see that the trees on the perimeter of the surrounding fields are very similar, but don't quite mirror each other. [if player is brave] However, after cutting down the oak tree, the trees on the perimeter of the surrounding fields create a mirror image.[end if][paragraph break] You can see two paths on either side of the clearing. West is Western Field, and East is Eastern Field. You can see the cabin up ahead. North is to the cabin's front door. The path South leads to the Campsite." Forest Clearing is north of campsite. West of Forest Clearing is Western Field. East of Forest Clearing is Eastern Field.  
...
Instead of going north from forest clearing:
	if player is timid: 
		say "You need to be focusing on getting Luigi something to drink! You can come back to the cabin after.";
	if player is courageous:
		say "It's not a good idea to go back there. The axe murderer is coming.";
	If player is brave:
		move player to cabin;
	If player is accomplished:
		move player to cabin;
...
Understand "Use [thing]" as using. Using is an action applying to one thing.


attic floor is a thing. It is undescribed. Understand "floor", and "floorboards" as attic floor. Description is "The floor is surprisingly made of solid concrete."
...
Skull is a thing. Skull is in forest clearing. It is undescribed. Description is "A skull that looks like it belonged to a large man, most definitely the axe murderer. On the frontal bone of the skull you can see a huge crack. It looks like it was made with a large tool. Could it have been the cause of death?" Understand "remains" as skull.
...
your Sleeping bag is an enterable supporter in tent. Description is "Some comfy sleeping bags." Sleeping bag is undescribed. Understand "sleeping bags" as sleeping bag. [Didn't know what a supporter was until I saw the code for ehamilton16's let freedom swim. Creds to them!]
...
Western Birch trees is scenery in western field. Description is "You see five identical birch trees (name: Western Birch Trees), all growing in a straight row. On one end of the row of trees, there is a tree stump." Understand "trees" as western birch trees. [It took a while to figure out, but for so long I didn't know how to have items with the same name in different rooms. I figured out that the trick is to have different names but the same 'understand' names.]
...
Luigi is a person in campsite. Luigi is undescribed. Description of Luigi is "Luigi is lounging around the campsite. He seems to be preoccupied with reading a strange book." Luigi is carrying ouija board. Luigi is undescribed.

[code from Nicole's game, how to get the NPC to follow you.]

An every turn rule: 
	if player is not timid:
		if player is not brave:
...
Every turn rule:
	If ouija board is open:
		move Board to location of the player. 
					      ...
					      [INSTEAD OF/ UNDERSTAND ACTIONS AND WHERE THEY WILL OCCUR]
...
Instead of opening ouija board:
	...
	If the player is accomplished: [THIS PART TOOK FOREVER TO FIGURE OUT. I DIDN'T KNOW HOW TO HAVE MULTIPLE CONDITIONS FOR AN INSTEAD OF ACTION. I didn't know how to make the chain with all of the moods, it took a ong time to figure out that a lot of it has to do with the margins.]
		If the player is carrying ouija board:
...
Instead of using ouija board:
	if player is carrying ouija board: 
		if player is persistent:
			say "You can ask the board certain questions. You can ask about 'Murderer', 'Remains', 'Paranormal Investigator'."
...
Instead of knocking: say "You hear a click. The ouija board can now be opened.";
...
Instead of examining canteen:
	If water is in canteen: 
		say "A canteen full of water."; [<----- I kept on forgetting these semi-colons, they're super important, but they always messed up my code. Now, I can't forget them.]
	Otherwise:
		say "It's empty."
...
Understand "drop [thing] in [thing]", "drop [thing] into [thing]", "drop [thing] onto [thing]","drop [thing] in [thing]", "throw [thing] into [thing]", "throw [thing] onto [thing]", "Toss [thing] in [thing]", "toss [thing] into [thing]", and "Toss [thing] onto [thing]" as throwing in. Throwing in is an action applying to two things. [My brother and Brennan suggested that I make sure I have all of the possible things that people could say in the understnad portion of my code.]
...
Instead of taking wooden key: 
	say "After taking the wooden key, you notice that the planchette begins to move on its own. It spells 'You are brave... Now go...'[paragraph break] For once, you really do feel brave. And even stranger, you feel inclinced to listen to the spirit.";
...
Instead of reading ouija book:
	...
	Otherwise: 
		say "An odd, worn book. The cover reads: [italic type] Beware of Ouija. [roman type] There seem to be four pages in the Ouija Book, though many of the words are smudged and indecipherable. Pick up the book first, then start by reading page 1 of Ouija Book."
		

[In Attic:]
Understand "read paper" as reassuring. Instead of reassuring: say "You read, 'Search the pond. There you will find the remains. You must burn the remains if you want to survive.' Is this a dying message from the paranormal investigator?" Reassuring is an action applying to one topic.
...
Instead of smelling bed: say "Smells like something died."
									...
									[CODE TO DISABLE TAKE ALL]
									

[Got this code (below) after searching for ways to disable take all. From user 'Khelwood' on Google Groups. This code uses new grammar to make sure that you can't use an action like take with multiple objects. Now when someone enters 'take all', it should just say 'you can't use multiple objects with that verb.'] 
...
Understand "pick up [thing]" or "pick [thing] up" as taking.
									...
									[CODE FOR TABLE OF SONGS]
										

Instead of knocking on a topic listed in the Table of Songs, say "[song entry][line break]". [This code is the same one that was shown to me by Mrs. Kiang for my last game. I thought I would include it by incorporating it into the knocking on the ouija board, but I just kept the title "table of songs" because it worked antway and I didn't really know what else to call it.]
...
[CODE SOURCE FROM JIM AIKIN'S INFORM HANDBOOK] The attic door is north of the cabin. The attic door is south of the attic. The attic door is a door. The attic door is scenery.
...
[This code was inspired by Marissa Takebayashi's Ragnarok, and the code is from the Inform Recipe Book.] ouija book is a thing. ouija book is in cabin. It is undescribed. Description is "An odd, worn book. The cover reads: [italic type] Beware of Ouija. [roman type] There seems to be four pages in the Ouija Book, though many of the words are smudged and indecipherable. Start by reading page 1 of Ouija Book."
...
Instead of asking Board about "the spirit": say "You place your hands on the planchette, and immediately after asking the question, the planchette smoothly moves to each of the four corners of the board. Then, after a moment, the planchette spells 'i am here'. [paragraph break]Luigi: No way! I thought ouija boards were just toys.[paragraph break] You can ask the board about the murderer, the paranormal investigator, or the remains."

Instead of asking Board about "spirit": say "You place your hands on the planchette, and immediately after asking the question, the planchette smoothly moves to each of the four corners of the board. Then, after a moment, the planchette spells 'i am here'. [paragraph break]Luigi: No way! I thought ouija boards were just toys.[paragraph break] You can ask the board about the murderer, the paranormal investigator, or the remains."

Instead of asking Board about "The Murderer": say "the planchette spells the words 'i am'. [paragraph break]Luigi: Dude... We're talking to the spirit of the axe murderer![paragraph break] You can ask the board about the paranormal investigator or the remains."

Instead of asking Board about "Murderer": say "the planchette spells the words 'i am'. [paragraph break]Luigi: Dude... We're talking to the spirit of the axe murderer![paragraph break] You can ask the board about the paranormal investigator or the remains."

Instead of asking Board about "the paranormal investigator": say "This time, almost excitedly, the planchette spells 'i killed him'. Then, 'axed'. [paragraph break]Luigi: ........[paragraph break] You can ask the board about the remains."

Instead of asking Board about "paranormal investigator": say "This time, almost excitedly, the planchette spells 'i killed him'. Then, 'axed'. [paragraph break]Luigi: ........[paragraph break] You can ask the board about the remains."
...
Instead of asking Board about "remains": say "The planchette does not move for a while. Then, it slowly spells 'make a mirror image'. [Paragraph break]Luigi: What does it mean, make a mirror image? What is it asking us to do? Ask the board about the mirror image."
...
Instead of asking Board about "hint": say "The planchette spells out 'Axes make mirrors. Life and death are mirrors. Young and old, rich and poor. [italic type] East and West are mirrors as well. [Paragraph break][roman type]Luigi: Something about East and West???"

Instead of asking Board about "axe": say "The planchette spells 'find it'.[paragraph break] Luigi: Gee, that's helpful..."

Instead of asking Board about "the axe": say "The planchette spells 'find it'.[paragraph break] Luigi: Gee, that's helpful..."
...
Check talking to Luigi: 
	...
	If player is timid: 
		say "Luigi: Yo! Feel free to ask me about stuff. [paragraph break] You can ask Luigi about Tent, Campsite, Object, Himself, Yourself, etc.";
	If player is courageous:
		say "Luigi: We have to find the remains and burn them before that crazy skeleton kills us!";
	If player is persistent:
		say "Luigi: This is getting kinda creepy.... For you, right? I'm not scared at all.";
	If player is brave:
		say "Luigi: Are we really going to do what that spirit says? I know that you're trying to prove to everyone you're brave and all, but... Come on, it's the spirit of the axe murderer!"
...
[code from get that cat]

Instead of throwing:
	if player is courageous: 
		say "As you near the flames, you feel a growing nervousness and fear of the outcome. The planchette angrily moves rapidly from each of the corners of the board. The skeleton is almost upon you. Are you sure?";
		if player consents:
			say "You finally bring yourself to push away your fears, and throw the skull into the fire. The skeleton falls to pieces, and the planchette lays frozen on the board.[paragraph break] Luigi: You did it... You saved us! I guess I have to admit you're not some big scaredy-cat after all.";
			end the story finally saying "GOOD END."
		
Understand "destroy [thing]", "torch [thing]", "burn [thing]" as conquering. conquering is an action applying to one thing.
Instead of conquering:
	...
	Otherwise:
		say "You are no where near a fire."
		
Instead of cutting ouija board:
	if player is carrying axe:
		if player is tested:
			say "With a single blow, you chop the Ouija Board in half. You have destroyed the board. You remember what you read on the final page of the diary-- something about not destroying the board, whatever you do. Regardless, you unleashed the evil spririt of xxxxxxxx xxx, and unwittingly sent yourself and Luigi to your deaths.";
...
Understand "Jump out of window", "Jump through window", "Escape through window", "Jump through the window", "Jump out of the window", "Jump out the window", "Jump out window", and "escape through the window" as jumping. 
Instead of jumping: 
	if player is tested:
...
Descision time is a scene. Descision time begins when player is tested. Descision time ends when player is courageous. [This code is from the inform recipe book. I wanted to have time, but not have it start when the game begins. So, I created a scene that would trigger the clock. Very happy with how this coding turned out, makes the game much more suspenseful.]
...
When descision time begins: 
	now the time of day is 2:30 am;
	now the right hand status line is "[time of day]".
At 2:32 am:
	say "The possessed skeleton is quickly advancing towards you. Hurry!"
At 2:34 am:
	say "The skeleton is almost upon you...."
At 2:37 am:
	if player is not courageous:
		say "You were too slow, and you were unable to escape the spirit of the axe murderer. You died afraid.";
		end the story finally saying "BAD END."
At 2:38 am:
	say "The skeleton is relentlessly chasing after you. The sounds of its footsteps grow louder. It sounds like it's swinging something-- the axe you dropped?"
At 2:40 am:
	say "You see the skeleton emerging from behind several trees, swinging the rusted axe as the spirit possessing it did in life. Hurry, before it's too late!"
At 2:45 am:
	say "You were too slow, and you were unable to escape the spirit of the axe murderer. You died afraid.";
	end the story finally saying "BAD END."
